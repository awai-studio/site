...
export async function validateArticleImage(file) {
  ...
  if (!file.name || file.name.length > ARTICLE_IMAGE_LIMITS.filename) {
    return invalid("写真のファイル名が長すぎます。");
  }

  if (CONTROL_CHARACTER_PATTERN.test(file.name)) {
    return invalid("写真のファイル名に使用できない文字が含まれています。");
  }

  if (file.size > ARTICLE_IMAGE_LIMITS.size) {
    return invalid("写真は4MB以内に調整してください。");
  }

  const imageType = IMAGE_TYPES[file.type];
  if (!imageType || !imageType.filenamePattern.test(file.name)) {
    return invalid("写真はJPEG、PNG、WebP形式で選択してください。");
  }

  try {
    const bytes = new Uint8Array(await file.slice(0, 12).arrayBuffer());
    if (bytes.length < 12 || !imageType.matches(bytes)) {
